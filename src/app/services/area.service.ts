...
export class AreaService {
  ...
  constructor() {
    ...
    this._subject = new BehaviorSubject<Area[]>(this._areas);
    this._subject.subscribe((areas: Area[]) => window.localStorage['area'] = JSON.stringify(areas));
  }
  ...
  save(areas: Area[]) {
    areas = areas
      .filter(area => (area.label && area.city))
  ...
  delete(id: string) {
    const result = this._areas.findIndex((area: Area) => (area.id === id));
    if (result !== -1) {
